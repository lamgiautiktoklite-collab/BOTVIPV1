...
/**
 * Giải mã ngày tạo từ TikTok ID (Fix lỗi năm 2068/2079)
 */
...
// Lệnh khởi đầu
bot.start((ctx) => ctx.reply('Chào mừng! Gửi link TikTok để tải video hoặc gõ /tt [username] để xem info.'));

// Xử lý lệnh /tt (Xem thông tin user)
...
╰─────────────⭓`;
            ...
            await ctx.reply('❌ Không tìm thấy người dùng này.');
        }
    } catch (err) {
...
// Tự động bắt link TikTok để tải video
bot.on('text', async (ctx) => {
    const text = ctx.message.text.trim();
    if (!text.includes('tiktok.com')) return;

    try {
        await ctx.reply('⏳ Đang xử lý video...');
        ...
        if (res.data.code === 0) {
            ...
            await ctx.reply('❌ Link hỏng hoặc video riêng tư.');
        }
    } catch (e) {
